...
impl Display for MMCLLError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MMCLLError::LaunchAccountAccessToken => write!(f, "账号AccessToken错误（仅在登录微软正版时触发）"),
            MMCLLError::LaunchAccountNoLegal => write!(f, "账号未购买正版（仅在登录微软正版时触发）"),
            MMCLLError::LaunchAccountThirdpartyAccessTokenOrURL => write!(f, "账号第三方的AccessToken或者URL错误。（仅在登录第三方时触发）"),
            MMCLLError::LaunchAccountThirdpartyBase => write!(f, "账号base64编码错误（仅在登录第三方时触发）"),
            ...
            MMCLLError::LaunchAccountAuthlib => write!(f, "Authlib-Injector.jar未找到。（仅在登录第三方时触发）"),
            MMCLLError::GameEligibleJsonNotFound => write!(f, "无法在版本文件夹中读取到版本JSON。（需要重新下载一遍MC）"),
            MMCLLError::GameInheritsFromVersionLose => write!(f, "已从版本json中找到inheritsFrom键，但是无法找到依赖版本。（需要重新下载一遍MC）"),
            MMCLLError::GameCannotUnzipNative => write!(f, "无法解压natives文件，请重试！"),
            MMCLLError::GameInheritsJsonStructure => write!(f, "已获取到inheritsFrom键下的版本json，但是结构不对。（比如找不到mainClass等。）"),
            ...
            MMCLLError::LoginTimeout => write!(f, "微软登录，登录超时（15分钟未完成登录），请重试！"),
            MMCLLError::LoginRefreshTokenExpire => write!(f, "微软登录，刷新密钥也同样过期了。"),
            MMCLLError::LoginXboxLiveInvalid => write!(f, "在进行xbox登录时出现了错误，可能是没挂vβn的原因。"),
            MMCLLError::LoginXstsLiveInvalid => write!(f, "在进行xsts登录时出现了错误，可能是没挂vβn的原因。"),
            MMCLLError::LoginXstsNoXbox => write!(f, "在进行xsts登录时，由于该账户没有xbox账号，你可能需要自己注册一个。"),
            MMCLLError::LoginXstsIllegal => write!(f, "在进行xsts登录时，由于该国家/地区被禁止，无法登录。"),
            MMCLLError::LoginXstsNoAdult => write!(f, "该账号需要成人验证（韩国）。"),
            MMCLLError::LoginXstsUnder18 => write!(f, "该账号设置未满18周岁，需要成人将该账户添加到家庭组中。"),
            MMCLLError::LoginXboxXstsUsercode => write!(f, "你请求的xbox usercode与xsts usercode二者不一致，请重新尝试！"),
            MMCLLError::LoginMcInvalid => write!(f, "在进行mc登录时出现了错误，可能是没挂vβn的原因。"),
            ...
            MMCLLError::LoginAccessTokenExpire => write!(f, "第三方登录，用于刷新的登录密钥已过期很久。"),
